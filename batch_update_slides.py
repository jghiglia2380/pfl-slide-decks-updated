...
def extract_story_and_question(soup):
    """Extract story summary and framing question from the teacher guide"""
    # Look for the first flow-item (THE CHALLENGE section)
    flow_items = soup.find_all('div', class_='flow-item')
    if flow_items:
        first_item = flow_items[0]
        flow_content = first_item.find('div', class_='flow-content')
        if flow_content:
...
def update_chapter(chapter_num):
    ...
    json_path = f'slide-content/L-{chapter_num}.json'
    ...
    print(f"  ✓ Updated L-{chapter_num}.json")
    return True

def main():
    """Process all chapters from L-15 to L-28"""
    print("=" * 60)
    print("BATCH SLIDE ENRICHMENT: L-15 through L-28")
    print("=" * 60)

    success_count = 0
    fail_count = 0

    for chapter_num in range(15, 29):
        try:
